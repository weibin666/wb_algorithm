...
暂时无法在飞书文档外展示此内容
理解题目
本题要求在一个以缩进格式描述的树形结构中，找到所有从根节点到叶子节点的路径中，能通过删除若干字符（保持相对顺序）形成目标符号序列的最短路径，并返回字典序最小的那一条。
具体地说，每行的缩进数量（由 |- 的数量决定）表示当前节点的深度，整棵树是按照深度优先顺序遍历给出的。
 路径中可以跳过部分节点，只要剩下的节点能够按顺序匹配目标符号序列即可。
需要注意的约束有两点：
- 匹配的子路径不要求连续，只需保持相对顺序一致；
- 若有多个合法子路径，优先返回最短的，若长度相等则返回字典序最小的。
使用算法
本题核心使用的算法包括：
1. 树结构的遍历与构造（隐式）
 输入数据是深度优先遍历结果，层级关系由 |- 的数量推断，不需显式构造树。
 使用一个栈维护当前路径，每读入一个节点，根据其缩进层级弹出或保留已有路径。
2. 贪心+双指针匹配子序列
 为判断某条路径是否匹配目标序列，使用双指针方法，按顺序扫描目标字符串，在路径中寻找匹配字符。
 这等价于判断一个序列是否是另一个序列的子序列（subsequence），时间复杂度为 O(m)，其中 m 为路径长度。
3. 最优解筛选
 用一个变量维护当前最优路径，比较标准为：
  - 路径长度更短；
  - 若长度相同，则字典序更小。
...
实现过程可以分为以下几个关键步骤：
1. 解析输入并维护路径
每读入一行节点，首先统计 |- 的数量，得到当前节点的深度（记作 cnt）。 然后维护一个栈结构 st，表示当前路径：
- 若栈深度超过当前节点深度，说明应从上一分支返回，弹出多余节点；
- 将当前节点加入栈中，构成从根到当前节点的路径。
这样每次栈中的内容都表示一条从根出发的路径。
2. 判断路径是否包含目标序列（作为子序列）
编写一个辅助函数，使用双指针判断当前路径 st 中是否包含目标序列 t 作为子序列。
若匹配成功，再反向处理该路径，从末尾向前找到能匹配目标序列的最短部分，这一步也是用双指针完成，能保证路径最短。
3. 筛选并更新最优答案
对于每一个匹配成功的路径片段：
- 若这是第一个匹配成功的路径，则记录；
- 若比之前的路径更短，更新答案；
- 若长度相同但字典序更小，也更新答案。
最终，输出满足条件的最短合法路径；若没有合法路径，则输出字符串 "null"。
